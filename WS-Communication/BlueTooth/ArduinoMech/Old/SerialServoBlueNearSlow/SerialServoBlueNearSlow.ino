...
int stopServo;         // center servo position  
...
void setup() {  
  ...
  stopServo = maxPulse - ((maxPulse - minPulse)/2);  
  pulseWidth = 1500;//centerServo;   // Give the servo a starting point (or it floats)  
  Serial.begin(115200);  
  //Serial.println("      Arduino Serial Servo Control");  
  //Serial.println("Press < or > to move, v to center");  
  //Serial.println();  
}  
...
void loop() {  
  ...
  if (Serial.available() > 0) {  
   ...
   if (moveServo == 118) { pulseWidth = stopServo; }
   if (moveServo == 49) { 
           pulseWidth = 1200;
