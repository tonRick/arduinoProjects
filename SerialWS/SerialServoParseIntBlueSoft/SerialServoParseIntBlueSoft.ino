...
void setup()
{
  ...
  mySerial.begin(115200);//Start soft serial comm
...
void loop ()
{
  if (mySerial.available() >0 ) //This command ensures that data is available
  {
    angle  = mySerial.read(); //if there is  data, get it!
  }
  
  //now, need to constrain the angle so that the servo doesn't break
  angle = constrain(angle,0,180);
  servo1.write(angle); //write the angle
  mySerial.println(angle); //So we can see the data on the Serial monitor
