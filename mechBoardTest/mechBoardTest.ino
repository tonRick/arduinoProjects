...
//Since the pullup is activated, we will look for low to detect pushbutton
  ...
  DDRB  =   0b00001111; // Pin 0 on Port B output setup
...
//IMPORTANT NOTE: In order for the pullups to remain active, the PORTB pin 4 needs to remain at 1 throughout! 
    if (PINB & (1<<4)){
      PORTB = 0b00011110;          /* Turn on pin3 (500hz red), pin2(RGBred) and pin1(RGBgreen) on PORTB */
      _delay_us(100);                                           /* wait - tested to give ~20% green so orange is output*/
